...
## 1. 숫자를 문자로 변환 {#nlp-number-to-text}
...
## 2. 숫자 표식 {#nlp-add-text-to-number}

숫자의 가독성을 높이고자 `%`와 같은 문자를 숫자와 함께 표현한다.
...
## 3. 표 작성 {#nlp-number-table}

원화 유니코드는 `\u20A9`으로 이를 반영하여 간단한 표를 구현하는 것도 가능하다.

``` {r string-table-ex, warning=FALSE, message=FALSE}
# 4. 숫자표 생성 ------------------------
