...
## 1. 텍스트/문자열을 다루는 R 툴체인 {#nlp-stringr-toolchain}
...
## 2. 문자열 기본 작업 {#nlp-stringr-basic-operation}

### 2.1. 문자열 찾고, 문자열 쪼개고, 문자열 치환 {#nlp-stringr-detect}
...
## 3. 정규(표현)식 {#nlp-regular-expression}
...
### 3.1. `rebus` 정규식 {#nlp-regular-expression-rebus}
...
### 3.2. 한글 정규식 {#nlp-regular-expression-hangul}
...
### 3.3. 포획(Capture) {#nlp-regular-expression-capture}
...
### 3.4. 역참조(back reference) [^regex-blog] {#nlp-regular-expression-back-reference}
...
## 4. 정규표현식 사례 [^regex-email] {#nlp-regular-expression-example}
