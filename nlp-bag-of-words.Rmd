...
## 1. 단어주머니 기반 지식 {#bag-of-words-basic-knowledge}

### 1.1. 언어기술 로드맵 {#language-roadmap}
...
### 1.2. 단어주머니 접근법 {#language-bag-of-words-approach}
...
### 1.3. 텍스트 데이터 분석 작업 흐름 {#language-text-analysis-workflow}
...
#### 1.3.1. 텍스트 데이터 전처리 [^text-mining-in-r] {#language-text-analysis-workflow-01}
...
#### 1.3.2. 문서단어행렬(DTM), 단어문서행렬(TDM)  {#language-text-analysis-workflow-02}
...
## 2. 텍스트 분석 상세 작업흐름도 {#nlp-r-workflow}
...
### 2.1. `qdap`, `tm` 텍스트 자료구조 비교 {#nlp-tm-qdap}
...
### 2.2. `tm`, `qdap` 데이터 정제 기능 {#nlp-tm-qdap-cleansing}
...
### 2.3. 텍스트에서 말뭉치(Corpus)로 변환 {#nlp-r-text2corpus}
...
### 2.4. 말뭉치 정제  {#nlp-r-corpus-cleansing}
...
### 2.5. 단어문서행렬(Term Document Matrix, TDM) {#nlp-r-corpus-tdm}
...
## 3. 텍스트 데이터 시각화 [^obama-farewell] [^trump-inauguration] {#nlp-r-obama-viz}
...
### 3.2. 데이터 불러오기 {#nlp-r-obama-import}
...
### 3.3. 데이터 전처리 {#nlp-r-obama-preprocess}
...
### 3.4. 텍스트 데이터 시각화 {#nlp-r-obama-word-viz}
