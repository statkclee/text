...
## 2. 영화 데이터 분석 {#nlp-movielens-data-analysis}

### 2.1. 무비렌즈 데이터 {#nlp-movielens-data}
...
### 2.2. 무비렌즈 데이터 전처리 {#nlp-movielens-preprocessing}
...
## 3. 탐색적 데이터 분석 {#nlp-movielens-eda}
...
### 3.1. 연도별 출하되는 영화는 몇편이나 될까? {#nlp-movielens-eda-01}
...
### 3.2. 각 쟝르별로 출하되는 영화편수는 어떨까? {#nlp-movielens-eda-02}

``` {r movielens-eda-moives-by-genre-year, eval=FALSE}
## 3.1. 연도별 영화 쟝르 출하 분석 --------------------------------
movies_df %>%
...
### 3.3. 사용자 평점에 기초한 시대별 최고 영화는 무엇일까? {#nlp-movielens-eda-04}

``` {r movielens-eda-moives-by-decades, eval=FALSE}
...
### 3.4. 사용자 평점에 기초한 쟝르별 최고 영화는 무엇일까? {#nlp-movielens-eda-05}

``` {r movielens-eda-moives-by-genre, eval=FALSE}
...
## 3.5. 각 쟝르를 특정 짓는 태그는 어떤 것이 있을까? {.tabset .tabset-fade .tabset-pills} 
