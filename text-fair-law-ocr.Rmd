...
```{r ocr-evaluation-fair-law}

fair_ocred_book  <-  read_rds("data/ocr-fair-azure.rds")

fair_azure_df <- fair_ocred_book %>% 
  enframe() %>% 
  mutate(temp_text = map(value, unlist)) %>% 
  mutate(text = map_chr(temp_text, paste0, collapse="")) %>% 
  bind_cols(fair_pdf_df) %>% 
  select(page, text)
...
save_ocr_text_png <- function(page) {
  ...
  annotated_png <- white_png %>% 
    image_annotate(fair_azure_df$text[page], location = "+100+100", size = 12, font = "AppleGothic")
  ...
  image_write(annotated_img, glue::glue("fig/fair_ocr_azure/fair_ocr_{page}.png"))
}
...
white_png <- image_blank(550, 756, color = "white", pseudo_image = "", defines = NULL)
annotated_png <- white_png %>% image_annotate("대한민국", size = 15, font = "AppleGothic")
annotated_png
